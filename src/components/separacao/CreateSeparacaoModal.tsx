...
import { X, Tag, FileText as FileTextIcon, User, Phone, MapPin, Calendar, RefreshCw, Check, Loader2, Table2, Image, FileText, Clipboard } from 'lucide-react';
...
interface CreateSeparacaoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function CreateSeparacaoModal({ isOpen, onClose, onSuccess }: CreateSeparacaoModalProps) {
  const { createSeparacao, generateNextCode, uploadMaterial, isSubmitting } = useCreateSeparacao();
  ...
  const validateForm = (): boolean => {
    ...
    if (!dataEntrega) {
      ...
      if (selectedDate < today) {
        newErrors.data_entrega = 'Data deve ser futura';
      }
    }
    ...
    } else if (materialMethod === 'digitar' && items.length === 0) {
      newErrors.material = 'Adicione pelo menos 1 item';
    } else if ((materialMethod === 'pdf' || materialMethod === 'imagem') && !materialFile) {
      newErrors.material = 'Selecione um arquivo';
    } else if (materialMethod === 'colar' && items.length === 0) {
      newErrors.material = 'Processe e confirme a lista';
    }
  ...
  const handleSubmit = async () => {
    ...
    if ((materialMethod === 'pdf' || materialMethod === 'imagem') && materialFile) {
      ...
      try {
        materialConteudo = await uploadMaterial(materialFile, codigoObra, materialMethod);
    ...
    const success = await createSeparacao({
      ...
      items: (materialMethod === 'digitar' || materialMethod === 'colar') 
  ...
  const handleClose = () => {
    const hasData = cliente || responsavel || telefone || endereco || items.length > 0 || materialFile;
    if (hasData) {
  ...
  const isFormValid = () => {
    return (
      ...
      dataEntrega &&
      materialMethod &&
      ((materialMethod === 'digitar' && items.length > 0) ||
       (materialMethod === 'colar' && items.length > 0) ||
       ((materialMethod === 'pdf' || materialMethod === 'imagem') && materialFile))
    );
  };
      ...
      <DialogContent className="max-w-[900px] max-h-[90vh] overflow-y-auto p-0">
        <DialogHeader className="p-6 pb-4 border-b sticky top-0 bg-background z-10">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-xl font-bold">Nova Separação</DialogTitle>
          </div>
        </DialogHeader>
